...
//     $scope.rollDice = function(sides) {
//         $scope.diceResult = Math.floor(Math.random() * sides) + 1;
//     };
// }]);

app.controller('diceRoller', ['$scope', function($scope) {
    console.log('Controller is loaded');
    
    $scope.diceResult = '';

    $scope.rollDice = function(sides) {
        console.log('Rolling dice with ' + sides + ' sides');
        $scope.diceResult = Math.floor(Math.random() * sides) + 1;
    };
}]);